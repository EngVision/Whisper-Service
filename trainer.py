...
def getTrainer():
    ...
    model, decoder = models.getASRModel()
    model = model.to(device)
    model.eval()
    asr_model = models.NeuralASR(model, decoder)

    phonem_converter = models.EngPhonemConverter()
...
class PronunciationTrainer:
    ...
    def __init__(
        self,
        asr_model: interfaces.IASRModel,
        word_to_ipa_coverter: interfaces.ITextToPhonemModel,
    ) -> None:
    ...
    def processAudioForGivenText(
        self, recordedAudio: torch.Tensor = None, real_text=None
    ):
        start = time.time()
    ...
    def getAudioTranscript(self, recordedAudio: torch.Tensor = None):
        ...
        ) = self.getTranscriptAndWordsLocations(current_recorded_audio.shape[1])
    ...
    def getWordLocationsFromRecordInSeconds(
        ...
        for word_idx in range(len(mapped_words_indices)):
            start_time.append(
                float(word_locations[mapped_words_indices[word_idx]][0])
                / self.sampling_rate
            )
            end_time.append(
                float(word_locations[mapped_words_indices[word_idx]][1])
                / self.sampling_rate
            )
    ...
    def matchSampleAndRecordedWords(self, real_text, recorded_transcript):
        ...
        mapped_words, mapped_words_indices = utils.get_best_mapped_words(
    ...
    def getPronunciationAccuracy(self, real_and_transcribed_words_ipa) -> float:
        ...
        for pair in real_and_transcribed_words_ipa:
            ...
            number_of_word_mismatches = utils.edit_distance_python(
